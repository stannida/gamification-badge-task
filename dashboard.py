...
action_columns = ['POSTS_CREATED','REPLIES_RECEIVED','THANKYOUS_RECEIVED','EVENTS_CREATED','EVENT_PARTICIPANTS','ITEMS_GIFTED','PLACES_RECOMMENDED']
...
You can use the following widgets to select a subset of data for exploration. The dashboard will adapt instantly. 
    ...
    if action_select!='All':
        ...
        if not not_null_select:
            week_action_select_df = week_action_select_df[(week_action_select_df[dist_columns] != 0).any(axis=1)]

    st.write(f"Distribution(s) of the number of actions performed by users in the last {week_select} weeks:")
    ...
    for i, var in enumerate(dist_columns):
        fig.add_trace(
            go.Box(y=week_action_select_df[var],
            name=var),
            row=1, col=i+1
        )
    ...
    st.write("Percentage of users with at least 1 action of each type out of total number of users in the last N weeks.")
    ...
    badge_select = col1.selectbox(
                                ...
                                ('Event Planner', 'Conversation Starter', 'Philanthropist','Helping Hand','Local Guide'))
    user_pct_filter = col2.slider(label="top % of users to award the badge to", 
                                  min_value=1,
                                  max_value=50,
                                  value=20,
    ...
    if badge_select=='Event Planner':
        main_action_column, second_action_column, second_column_limit = 'EVENTS_CREATED', 'EVENT_PARTICIPANTS', 1
        badge_data = data
        ...
        main_action_column, second_action_column, second_column_limit = 'POSTS_CREATED', 'REPLIES_RECEIVED', 0
        ...
        most_common_action = 'created only 1 post with 0 replies out of all users who posted'
        action_metric_name = 'replies'
        action_verb = 'create'
    elif badge_select=='Philanthropist':
        main_action_column, second_action_column, second_column_limit = 'ITEMS_GIFTED', None, 0
        badge_data = data
        badge_description = 'This badge is given to users that give away items for free in given timeframe'
        most_common_action = 'gifted only 1 item of all users who gifted on the marketplace'
        action_verb = 'give away'
    elif badge_select=='Helping Hand':
        main_action_column, second_action_column, second_column_limit = 'THANKYOUS_RECEIVED', None, 0
        badge_data = data
        badge_description = 'This badge is given to users that receive thank you messages in given timeframe'
        most_common_action = 'received only 1 thank-you message of all users who received any'
        action_verb = 'receive'
    elif badge_select=='Local Guide':
        main_action_column, second_action_column, second_column_limit = 'PLACES_RECOMMENDED', None, 0
        badge_data = data
        badge_description = 'This badge is given to users that recommend places in given timeframe'
    ...
    pct_users_with_action, pct_users_with_one_action, action_df_per_week, action_df_cumm = get_action_stats(badge_data, week_filter, main_action_column, second_action_column, second_column_limit)

    st.markdown(f'''{badge_description}.  
...
**{pct_users_with_one_action}%** of all users {most_common_action} in the last 12 weeks.
    ...
    if second_action_column:
        participants_per_action_freq = badge_data[(badge_data[main_action_column]>0)&(badge_data['LAST_X_WEEKS']==week_filter)].groupby([main_action_column, second_action_column+'_AVG','LAST_X_WEEKS'])['USER_ID'].nunique().reset_index()
        participants_per_action_freq.rename(columns={"USER_ID":"Number of Users"},inplace=True)
        fig = px.scatter(participants_per_action_freq, x=main_action_column, y=second_action_column+'_AVG',
        ...
        participants_per_action_freq = badge_data[(badge_data[main_action_column]>0)&(badge_data['LAST_X_WEEKS']==week_filter)].groupby([main_action_column,'LAST_X_WEEKS'])['USER_ID'].nunique().reset_index()
        participants_per_action_freq.rename(columns={"USER_ID":"Number of Users"},inplace=True)
        fig = px.scatter(participants_per_action_freq, x=main_action_column, y='Number of Users')

    st.plotly_chart(fig,use_container_width=True,theme=None)

    #percentage of total users that will receive a badge
    badge_achievers_pct = np.round(action_df_cumm[action_df_cumm[main_action_column]>=action_df_top_pct[main_action_column].min()]['USER_ID'].nunique()*100/badge_data[badge_data['LAST_X_WEEKS']==week_filter]['USER_ID'].nunique(),1)

    if second_action_column:
        st.markdown(f'''For top {user_pct_filter}% of users to earn the badge they need to {action_verb} at least {action_df_top_pct[main_action_column].min()} actions with {action_df_top_pct[second_action_column+'_AVG'].min()} {action_metric_name} on average in {week_filter} weeks.  
    This way the bagde would be awarded to {badge_achievers_pct}% of all users.
                ''')
    else:
        st.markdown(f'''For top {user_pct_filter}% of users to earn the badge they need to {action_verb} at least {action_df_top_pct[main_action_column].min()} actions in {week_filter} weeks.  
    This way the bagde would be awarded to {badge_achievers_pct}% of all users.
                ''')
    st.divider()
